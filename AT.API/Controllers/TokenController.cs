...
namespace AT.API.Controllers
{
    ...
    public class TokenController : ControllerBase
    {
        ...
        public IActionResult Token([FromBody] TokenRequest request)
        {
            ...
            var user = this.UsuarioService.AutenticarUsuario(request.email, request.senha);

            if (user == null)
                return Unauthorized();

            return Ok(new { AccessToken = this.GenerateToken(user) });
        }
    ...
    public record TokenRequest(
        ...
        [Required(ErrorMessage = "Senha é obrigatório")] string senha);
}
